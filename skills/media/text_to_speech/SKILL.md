...
**【重要】当技能涉及用户输入数字（如语速 speed、音调 pitch 等）时，必须考虑全角数字的兼容性。**

在中文输入环境下，用户可能输入全角数字（如 `０．９`、`＋５`）而非半角数字（如 `0.9`、`+5`）。本技能已内置全角数字自动转换功能，无需 Agent 额外处理。

**涉及全角数字兼容性的参数：**
- `speed`（语速）：支持全角数字输入，如 `０．９`、`１．０`
- `pitch`（音调）：支持全角数字输入，如 `０`、`＋５`、`－５`
