...
description: 查询招标信息、采购公告、项目招标等招投标相关信息。通过关键词和省份筛选，获取招标标题和链接。支持固定触发方式：招标搜索：关键词。
category: network
requires_confirmation: false
version: "2.0"
author: XuanShu Agent
tags: ["招标", "采购", "招投标", "信息查询", "网络搜索", "招标信息", "招标项目", "招标搜索", "政府采购", "工程招标", "项目招标"]
parameters:
  - name: "keyword"
    type: "string"
    description: "搜索关键词（必填），如：设备采购、工程建设、软件开发、轴承、空调。支持通过'招标搜索：关键词'方式触发。"
    required: true
  - name: "province"
    type: "string"
...
- **招标信息查询**：用户要求"查一下最近有哪些软件开发的招标"、"北京地区的工程招标信息"、"查询关于轴承的招标项目"、"查询关于空调的招标信息"
- **搜索招标信息**：用户说"搜索招标信息"、"帮我搜索招标信息"、"找一下招标信息"
- **查找招标项目**：用户说"查找招标项目"、"帮我找招标项目"、"搜索招标项目"
- **网上找招投标**：用户说"网上找找招投标"、"去网上找找招标信息"、"网上搜索招标"
- **固定触发方式**：用户说"招标搜索：关键词"，如"招标搜索：轴承"、"招标搜索：空调"
- **采购公告查询**：用户要求查询政府采购、采购公告、采购信息
- **市场调研**：了解某个行业的招标动态和竞争情况
- **商机发现**：寻找特定领域的采购机会
- **竞品监控**：监控竞争对手参与的招标项目
- **行业分析**：分析某个地区的招标趋势和规模

**判断标准**：当用户输入内容包含以下任一关键词或表达时，**必须**使用此技能：
- "招标" + "搜索/查找/查/找/查询/看看/找找"
- "搜索/查找/查/找/查询" + "招标/招投标/采购/项目"
- "招标搜索："（固定触发格式）
- "网上" + "找找/搜索/查" + "招标/招投标"
- "帮我" + "搜索/查找/查/找" + "招标/招投标/采购"

**重要提示**：只要用户表达与搜索/查找招标信息、招标项目、招投标、采购公告相关的意图，无论具体措辞如何，都必须调用此技能！

## Parameters

| Name     | Type    | Description                                         | Required | Default |
| -------- | ------- | --------------------------------------------------- | -------- | ------- |
| keyword  | string  | 搜索关键词（必填），如：设备采购、工程建设、轴承    | Yes      | -       |
...
- **设备采购**：设备采购、仪器采购、办公设备、医疗设备、轴承、空调
...
- **v2.0 更新**：大幅扩大触发范围，新增固定触发方式"招标搜索：关键词"，明确各种触发场景的判断标准
