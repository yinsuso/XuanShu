"""
Hermes Agent 控制器技能。
通过 HTTP API 控制 Hermes Agent 执行任务。
Author: 破执
Date: 2026-05-15
"""
...
logger = get_logger('hermes_controller')

# 技能元数据
SKILL_NAME = "hermes_controller"
SKILL_DESCRIPTION = "控制 Hermes Agent 执行任务，通过 HTTP API 发送任务并获取结果。"
SKILL_TRIGGER = "当需要调用 Hermes Agent 执行独立任务、分布式计算或跨平台协作时使用。"
...
SKILL_PARAMETERS = [
    ...
    },
    {
        "name": "host",
        "type": "string",
        "description": "Hermes Agent 地址，默认 http://localhost:8000",
...
def execute(action: str, host: str = "http://localhost:8000", task_description: str = "",
            task_id: str = "", timeout: int = 30, **kwargs) -> str:
    """
    执行 Hermes Agent 控制操作。

    Args:
        action: 操作类型
        host: Hermes Agent 地址
    ...
    logger.info(f"Hermes 控制: action={action}, host={host}")
