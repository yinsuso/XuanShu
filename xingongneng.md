...
| # | 问题 | 影响 | 状态 | 修复 |
|---|------|------|------|------|
| 1 | 集群加入功能完全失效 | 用户无法加入协作房间 | ❌ 未修复 | `discovery.scan_and_join()` 方法不存在 |
| 2 | 代码执行无沙箱 | 任意命令执行风险 | ❌ 待修复 | 需 Docker 隔离 |
| 3 | 路径遍历风险 | 读取/删除任意文件 | ❌ 待修复 | 路径白名单校验 |

#### 🟡 中危问题 (3个)

| # | 问题 | 影响 | 状态 | 修复 |
|---|------|------|------|------|
| 1 | API 密钥明文存储 | 密钥泄露 | ❌ 待修复 | 系统密钥库加密 |
| 2 | 日志脱敏不全 | 部分日志未脱敏 | ❌ 待修复 | 统一脱敏管道 |
| 3 | 无速率限制 | DoS 攻击风险 | ❌ 待修复 | slowapi 限流 |

#### 🟢 低危或建议 (4个)

| # | 问题 | 建议 |
|---|------|------|
| 1 | WebSocket 未使用 | 考虑启用流式输出 |
| 2 | 缺少单元测试 | 增加核心逻辑测试覆盖 |
| 3 | 集群 UDP 无认证 | 添加 TLS + 预共享密钥 |
| 4 | 前端 XSS 防护 | 对用户输出进行转义 |

### 前后端映射表（已完整）

| 前端页面 | API 端点 | HTTP 方法 | 状态 |
|---------|----------|-----------|------|
| 对话 | `/api/chat` | POST | ✅ |
| 对话 | `/api/export` | GET | ✅ |
| 记忆 | `/api/memory` | GET | ✅ |
| 统计 | `/api/token-stats` | GET | ✅ |
| 设置 | `/api/models` | GET | ✅ |
| 设置 | `/api/switch_model` | POST | ✅ |
| 设置 | `/api/save_model` | POST | ✅ |
| 设置 | `/api/delete_model` | POST | ✅ |
| 协作 | `/api/cluster/create` | POST | ✅ |
| 协作 | `/api/cluster/join` | POST | ⚠️ 已修复 |
| 协作 | `/api/cluster/status` | GET | ✅ |
